(function () {
  ...
  function applyFirstPaint(items) {
    var newest = newestFirst(items);
    state.visible = newest.slice(0, MAX_ROWS).reverse();
    rememberAll(items);
    renderRows({ stickToBottom: true });
    persist();
  }

  function enqueueNew(items) {
    var fresh = newestFirst(items)
  ...
  function restore() {
    ...
    if (Array.isArray(lines) && lines.length) {
      state.visible = lines.slice(-MAX_ROWS);
      renderRows({ stickToBottom: true });
    }
